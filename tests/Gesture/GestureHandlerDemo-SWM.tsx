...
// Two measurements rarely agree to their last digit, so round both onto
// the pixel grid and compare those
const DENSITY = PixelRatio.get();

const toPixels = (dp: number) => {
    'worklet';
...
export default function NestedScrollView() {
    ...
    // only a scroll event knows where the list ends; until one arrives 
    // "far away" is the safe answer, where 0 would read as already docked
    const stickyTop = useSharedValue(Number.POSITIVE_INFINITY)
    ...
    const printLog = (...args: unknown[]) => {};


    const getLeftListState = (direction: 'up' | 'down'): E_ListScrollState => {
        'worklet';
    ...
    const getRightListState = (direction: 'up' | 'down'): E_ListScrollState => {
        'worklet';
    ...
    const scrollHandler = useAnimatedScrollHandler({
        onScroll: e => {
            ...
            outerScrollY.value = toPixels(e.contentOffset.y);
            // docked == the end of the list, which this event already describes
            // one source for both sides of the comparison
            stickyTop.value = toPixels(
